...
- **Cron script not found:** `no_agent` cron scripts must be **real files** (not symlinks) inside `$HERMES_HOME/scripts/`. Symlinks that resolve outside this directory are blocked. Use `cp` not `ln -sf`. Shell scripts (`.sh`) work for wrapping Python with `uv run`.
