...
def _research_poll(request_id: str, interval: int = 5, timeout: int = 180) -> dict[str, Any]:
    """Poll research endpoint until completion."""
    import time
        ...
        try:
            with urllib.request.urlopen(req, timeout=30) as resp:
                ...
                if status == "completed":
                    ...
                    sys.exit(1)
                print(f"  Research in progress... ({status})", file=sys.stderr)
        except urllib.error.HTTPError as e:
            print(f"Error polling research: {e.code} - {e.read().decode()[:200]}", file=sys.stderr)
            sys.exit(1)
        except Exception as e:
            print(f"Error polling research: {e}", file=sys.stderr)
            sys.exit(1)
        time.sleep(interval)
...
def cmd_research(args: argparse.Namespace) -> str:
    """Execute research command (async with polling)."""
    payload: dict[str, Any] = {
        "input": args.query,
    }
    ...
    # Submit research (async - returns immediately with request_id)
    print("  Submitting research request...", file=sys.stderr)
    data = _api_request("/research", payload, timeout=30)

    request_id = data.get("request_id") or data.get("id", "")
    if not request_id:
        print("Error: No request_id returned from research API", file=sys.stderr)
        sys.exit(1)

    # Poll for completion
    print(f"  Research ID: {request_id}", file=sys.stderr)
    result = _research_poll(request_id, timeout=args.research_timeout)
    return _format_research_results(result, as_json=args.as_json)
...
Examples:
  ...
  %(prog)s research "Impact of AI on healthcare"
    ...
    p_research.add_argument("--timeout", type=int, dest="research_timeout", default=180,
