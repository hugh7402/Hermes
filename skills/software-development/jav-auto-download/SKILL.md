...
1. **禁止 `-U`（无码破解版）**：文件名含 `-U`（非 `-UC`）或 `无码破解` 标注的磁链**一律禁止选择**。用户明确禁止下载无码破解版。
2. **内嵌中文字幕版优先**：文件名含 `-C`、`-UC`、`中字`、`内嵌`、`SUB`、`字幕` 等标记的磁链优先级最高。`-UC`（无码中字版）也是可接受的（因为它含中文字幕）。
3. **高清优先**：同类（同为带字幕版或同为裸版）中，选页面标注的文件大小最大的版本。
